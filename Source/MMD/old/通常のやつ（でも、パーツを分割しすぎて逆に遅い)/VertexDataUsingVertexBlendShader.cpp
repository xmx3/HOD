...
VertexDataUsingVertexBlendShader::~VertexDataUsingVertexBlendShader() {
	...
	for ( int i=0; i<numSubsets_; ++i) {
		// safe release vector
		for( 
		unsigned int j=0, size=vbBoneOfGroupInSubset_[i].size();
		j<size;
		++j )
		{
			SAFE_RELEASE( vbBoneOfGroupInSubset_[i][j] );
		}
		vbBoneOfGroupInSubset_[i].clear();
		vbBoneOfGroupInSubset_[i].shrink_to_fit();
	...
	const std::vector<CModel::Material*> &materials )
{
...
#ifdef USING_HLSL		
	//hr=sEffect->SetFloatArray("mViewProj", &dxMat._11, 16 );
	hr=sEffect->SetMatrix("mViewProj", &dxMat );
	assert( SUCCEEDED( hr ) );
#else
	//hr=g_pConstantTable->SetMatrix( pDevice, "mViewProj", &dxMat );
	hr=pDevice->SetVertexShaderConstantF( 47, &pvm.m00, 4 );  
	//hr=pDevice->SetVertexShaderConstantF( 47, &dxMat._11, 4 );  
	//hr=pDevice->SetVertexShaderConstantF( 47, &dxMat.m[0][0], 4 );//上と同じ  
	assert( SUCCEEDED( hr ) );
#endif
	...
	for (unsigned int subsetIdx=0; subsetIdx<subsets.size(); ++subsetIdx) {
		...
		for( int groupIdx=0; groupIdx<numGroups_[subsetIdx]; ++groupIdx ){
			...
			m.setVertexBuffer( vbOfGroupInSubset_[subsetIdx][groupIdx] );
			m.setIndexBuffer( ibOfGroupInSubset_[subsetIdx][groupIdx] );

			//ボーンのバーテックスバッファセット
			hr=pDevice->SetStreamSource(1, vbBoneOfGroupInSubset_[subsetIdx][groupIdx], 0, sizeof(VERTEX_BONE));
			assert( SUCCEEDED( hr ) );
			...
			for( UINT iPass = 0; iPass < numPasses; iPass++ )
			{
				...
				hr=pDevice->DrawIndexedPrimitive(
					D3DPT_TRIANGLELIST, 0, 0, vbOfGroupInSubset_[subsetIdx][groupIdx].vertexNumber(), 
					0, ibOfGroupInSubset_[subsetIdx][groupIdx].indexNumber()/3 );
				assert( SUCCEEDED( hr ) );
...
void VertexDataUsingVertexBlendShader::init( const std::vector<CModel::Subset*> &subsets ){
	...
	for (unsigned int subsetIdx=0; subsetIdx<subsetSize; ++subsetIdx) {
		...
		for( int i=0; i<numGroup; ++i ){
			...
			vbBoneOfGroupInSubset_[subsetIdx].push_back(vbb);
		}
			...
			// create new VertexBuffer
			{
					...
					++it, ++newIndex )
				{
					...
					for( int iBone=0; iBone<MODEL_BLEND_COUNT; ++iBone ){
						const std::set<TypeBoneIndex> &pattern = usingBoneIndexPatternOfGroupInSubset_[subsetIdx][groupIdx];
						unsigned int patternIdx=0;
						//置換
						for(
							std::set<TypeBoneIndex>::iterator itPattern=pattern.begin(), endPattern = pattern.end();  
						...
						const VERTEX_BONE &boneA=newVbBone[newIndex];
						const VERTEX_BONE &boneB=vbBone[oldIndex];
						assert(
	...
	int startIndexBufferIdx )
{
	...
	//assert( SUCCEEDED( hr ) );
	hr = pDevice->SetVertexShaderConstantF( 11, &matArray->m00, 3*12 ); 
	assert( SUCCEEDED( hr ) );
