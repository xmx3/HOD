...
#include "kztrCharacterController.h"
...
const btScalar CharacterController::sk_minAccelSpeed = 1.0;
const btScalar CharacterController::sk_minSpeed = 50.0;
//---------------------------------------------------------------------------------------
...
CharacterController::CharacterController (btPairCachingGhostObject* ghostObject,btConvexShape* convexShape,btScalar stepHeight, int upAxis)
{
	...
	m_maxSpeed=100000.0;
	m_accelNormalVel.setValue( 0.0f, 0.0f, 0.0f );
	m_accelSpeed=0.0f;
	m_coefficientOfAccelration=0.9f;//‰Á‘¬“xŒW”AŒ¸‘¬(Deceleration)ŒW”‚Ì•û‚ª–¼‘O‚¢‚¢H
	m_airResistance=0.01f;//‹ó‹C’ïRŒW”
	m_coefficientOfFriction=0.5f;//–€ŽCŒW”@“®–€ŽCŒW”‚Æ‚©ÃŽ~–€ŽCŒW”ì‚éH
...
//   printf("  dt = %f", dt);
   ...
   // quick check...
	if (!m_useWalkDirection  && m_wasOnGround && m_accelSpeed==0.0 && m_walkDirection.length2()==0.0 ) {
//      printf("\n");
		return;      // no motion
	}
