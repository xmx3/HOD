...
-- 開始処理
function startfunc()
	print("startfunc() start! in HatuneOfTheDead/gameinit.lua" )
	...
	-- 最初は画面シーンを設定する
	-- GS.scene = GameEngineIntroduction:new()
	GS.scene = Title:new()
	-- GS.scene = Lobby:new()
	-- GS.scene = Stage:new()
	GS.scene:init()
...
function endfunc()
	print("endfunc() start! in HatuneOfTheDead/gameinit.lua" )
...
function change_scene(scene_class, ...)
	-- print( type( scene_class ) )
	GS.next_scene_class = scene_class
	GS.next_scene_cfg = {...}
	--ここでwaitんでおいたほうがいい気がする。なければC側に関数を返すことなくluaを続けてしまうから それでバグになるかも
	-- self:wait(0)
end

function change_scene_by_name(scene_class_name, ...)
	-- print( type( scene_class ) )
	assert( type(scene_class_name)=="string" )
	local class = rawget(_G, scene_class_name)
	-- print( type( class ) )
	assert(class~=nil)
