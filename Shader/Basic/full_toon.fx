...
static bool spadd=false;;    // スフィアマップ加算合成フラグ
#define SKII1    1500
#define SKII2    8000
#define Toon     3
...
VS_ZValuePlot_OUTPUT ZValuePlot_VS( MMM_SKINNING_INPUT IN )
{
    VS_ZValuePlot_OUTPUT Out = (VS_ZValuePlot_OUTPUT)0;
		...
		float4 Pos =  MMM_SkinnedPosition(IN.Pos, IN.Normal, IN.BlendWeight, IN.BlendIndices, IN.SdefC, IN.SdefR0, IN.SdefR1);
...
{
	float color = ShadowMapTex.z/ShadowMapTex.w;
	return float4( color, color * color, 0.0f, 1 );
}
...
float4 BufferShadow_PS(BufferShadow_OUTPUT IN, uniform bool useTexture, uniform bool useSphereMap, uniform bool useToon) : COLOR
{
    ...
    TransTexCoord.y = (1.0f - IN.ZCalcTex.y)*0.5f;
		...
		//     return Color;
		// } else {
		// 	float4 shadow = tex2D( DefSampler, TransTexCoord );
		// 	float e_x2 = shadow.y;
